  ...
  cent_2023_run3(iConfig.getUntrackedParameter<bool>("cent_run3"))
{
  ...
  */
  ...
  if(!filename.IsNull())
    {
      edm::FileInPath fip(Form("Analyzers/SayanCMW/data/EFF/%s",filename.Data()));
	...
	{	  
	  ...
	  for(unsigned int m = 0; m < ntrack; m++)
	    {	  
	      ...
	      if (std::abs(pt_m - pt_n) < 0.5) continue ;
	      std::cout << pt_n << "   " << pt_m << std::endl ;
	      //associate pt cuts                                                                           
  ...
  for (auto const& trk : *trks)
    {
      ...
      //selected tracks
      if( charge == 0 ) continue;
      if( fabs(pterror) / pt <= 0.0 ) continue;                                           //Default cuts
      if( fabs(dzvtx / dzerror) > 3.0 ) continue;                                        //Default cuts
      if( fabs(dxyvtx / dxyerror) > 3.0  ) continue;                                     //Default cuts
      if( ( chi2ndof / hit_pattern.trackerLayersWithMeasurement() ) < 0 ) continue;   //Default cuts
      if ( iter_tk.numberOfValidHits() <= 0 ) continue;
      ...
      else
	{
      ...
      double fake = hFak3D->GetBinContent(hFak3D->GetXaxis()->FindBin(eta), hFak3D->GetYaxis()->FindBin(pt), hFak3D->GetZaxis()->FindBin(centBin));
